...
fileprivate final class Exporter {
    ...
    init?(asset: AVMutableComposition, outputUrl: URL, composition: AVVideoComposition, quality: Quality) {
        ...
        self.session.outputFileType = AVFileType(rawValue: "com.apple.quicktime-movie")
...
extension MergeConfiguration {
    ...
    static var custom: MergeConfiguration {
        return MergeConfiguration(frameRate: 30, directory: NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0], quality: Quality.high, placement: Placement.custom(x: 0, y: 0, size: CGSize(width: 100, height: 100)))
    }
}
